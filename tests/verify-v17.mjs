...
console.log('===== SA 源码静态组 =====');
{
  ...
  const bmon = R('systems/explore.js');   // v39（E364）：buildMonster 自 status-fx.js 迁入 explore.js，断言随迁
  bmon.includes('const dataElite = !!d.elite;') && bmon.includes('const e = dataElite || !!opts.elitePlus;') ? pass('SA11 elitePlus 入对象构造（A7；v39 迁 explore.js）') : fail('SA11 elitePlus', '');
  bmon.includes('const dataElite = !!d.elite;') && bmon.includes('* foeMul)') && bmon.includes('foeMul = (typeof Game') ? pass('SA12 elitePlus 不叠倍率（A7）；v38（E318）群邪环伺 foeMul 单点（v39 迁 explore.js）') : fail('SA12 倍率', '');
  ...
  battle.includes("this.dealToEnemy(B, st, dmg * 0.3, { src: 'attack' })") && battle.includes("this.dealToEnemy(B, st, this.myAtk(st) * 0.2, { src: 'skill' })") ? pass('SA23 伤害统计补全（E5；v39（E364）追伤/追雷落账随 dealToEnemy 单源）') : fail('SA23 统计', '');
  ...
  craft.includes('const cost = this.drawPrice(p);') && craft.includes('失败不计数 talRounds') ? pass('SA54 画符成本恒 1×、失败不烧轮次（E38；v39（E365）死阶梯删除）') : fail('SA54 画符', '');
  ...
  forge.includes('pickAffix(cands, grade, wMul = 1)') && gdata.includes('w: 35, minGrade: 2') ? pass('SA83 词缀权重+门槛（D7）；v38（E317）配比 wMul') : fail('SA83 词缀权重', '');
