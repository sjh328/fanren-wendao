...
const BeastSys = {
  ...
  async tame() {
    ...
    const rate = Utils.clamp(45 + p.attrs.luck * 2 - diff, 8, 85);
    const ok = await UI.popup({
      title: `驯服 · ${e.name}`,
    ...
    if (Utils.chance(rate)) {
      ...
      if (slotFull) {
        Log.add(`你以神识温言相抚，${e.name} 俯首帖耳……可惜兽栏已满，你只能为其解开封印，目送它遁入山林。（驯服心得已涨）`, 'info');
        p.attrs.luck = Math.min(10, p.attrs.luck);
      } else {
