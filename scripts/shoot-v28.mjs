/* v28 视觉验收截图：桌面 1440×900 ×3 + 移动 390×844 ×3（含抽屉/更多面板打开态） */
...
await sleep(400);
await clearTransient();
await page.screenshot({ path: 'gui-test-screenshots/v28-d-quest.png' });
await page.evaluate(() => { Game.actions['act-tab']({ tab: 'shop:market' }); });
await sleep(400);
await clearTransient();
await page.screenshot({ path: 'gui-test-screenshots/v28-d-shop.png' });
